/* finds longest pallindromic substring alternative of manacher  o(n)*/
class Solution {
public:
    #define MAXN 2000 
...
//STEP 1// 
	...
	* Check : if s[idx] X s[idx] already exists or not*/
	if(tree[tmp].insertEdg[s[idx]-'0'] != 0) 
	{ 
		// s[idx] X s[idx] already exists in the tree 
		currNode = tree[tmp].insertEdg[s[idx]-'0']; 
		return; 
	} 
	...
	// weight as s[idx] 
	tree[tmp].insertEdg[s[idx]-'0'] = ptr; 
...
//STEP 2// 
	...
	tree[currNode].suffixEdg = tree[tmp].insertEdg[s[idx]-'0']; 
} 
	...
	{ 
		...
		// mx=max(mx,tree[i].length);
		// cout << endl; 
	}
        for (int j=tree[idx].start; j<=tree[idx].end; j++) 
        {
            ans+=s[j];
        }
